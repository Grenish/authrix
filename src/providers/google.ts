...
interface GoogleOAuthConfig {
  ...
  ): Promise<OAuthUser & { tokens?: { access: string; refresh?: string; idToken: string } }> {
    const config = getGoogleOAuthConfig();
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('fetch')) {
          throw new Error('Network error during Google authentication. Please try again.');
        }
   ...
   */
  export async function validateGoogleIdToken(idToken: string): Promise<GoogleUserInfo | null> {
    try {
      const response = await fetch(`${GOOGLE_OAUTH_ENDPOINTS.TOKEN_INFO}?id_token=${idToken}`);
