...
export function createPrismaAdapter(options: PrismaAdapterOptions = {}): AuthDbAdapter {
	...
	return {
		async findUserByEmail(email: string): Promise<AuthUser | null> {
			...
			const user = await userModel.findUnique({ where: { email: normalizeEmail(email) } });
		...
		async findUserByUsername(username: string): Promise<AuthUser | null> {
			const client = await ensureClient();
			const userModel = getModel(client, modelNames.user);
			const user = await userModel.findUnique({ where: { username: normalizeUsername(username) } });
			return user ? pickUserFields(user) : null;
		},
			...
			try {
				const updated = await userModel.update({
					...
					data: {
					  ...
					  profilePicture: data.profilePicture?.trim?.() ?? data.profilePicture,
					},
				});
		...
		async getUserTwoFactorCodes(userId: string, type?: string): Promise<TwoFactorCode[]> {
			const client = await ensureClient();
			const twoFactorModel = getModel(client, modelNames.twoFactor);
			const where: any = { userId, isUsed: false, expiresAt: { gt: new Date() } };
			if (type) where.type = type;
			return await twoFactorModel.findMany({ where, orderBy: { createdAt: "desc" }, take: 10 });
		},
