...
function getGitHubOAuthConfig(): GitHubOAuthConfig {
  ...
  const redirectUri =
    process.env.GITHUB_REDIRECT_URI || process.env.GITHUB_OAUTH_REDIRECT_URI;
...
export async function handleGitHubCallback(
  ...
  try {
    ...
    const [userData, emailData] = await Promise.all([
      gitHubApiRequest<GitHubUser>(
        GITHUB_OAUTH_ENDPOINTS.USER,
        tokenData.access_token
      ),
      gitHubApiRequest<GitHubEmail[]>(
        GITHUB_OAUTH_ENDPOINTS.EMAILS,
        tokenData.access_token
      ),
    ]);
    ...
    if (!primaryEmail && !options.skipEmailVerification) {
      throw new Error(
        "No verified primary email found. Please verify your email on GitHub."
      );
    }
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes("fetch")) {
        throw new Error(
          "Network error during GitHub authentication. Please try again."
        );
      }
...
export async function getGitHubUserOrganizations(
  ...
  try {
    return await gitHubApiRequest(`${GITHUB_API_BASE}/user/orgs`, token);
  } catch (error) {
